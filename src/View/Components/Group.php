                    ...
                    <fieldset class="fieldset py-0">
                        ...
                        <div class="join">
                            @foreach ($options as $option)
                                <input type="radio" name="{{ $modelName() }}" value="{{ data_get($option, $optionValue) }}" aria-label="{{ data_get($option, $optionLabel) }}" @if(data_get($option, 'disabled')) disabled @endif {{ $attributes->whereStartsWith('wire:model') }} {{ $attributes->class([ "join-item btn [&:checked]:btn-neutral", "!border-l-base-100" => data_get($option, 'disabled') ]) }} />
                            @endforeach
                        </div>
